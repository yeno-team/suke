...
const TheaterItem = ({item, onSubmit, sources, create, key, deleteCreateItem}: {key: number, item: ITheaterItem, onSubmit?: () => void, sources: string[], create?: boolean, deleteCreateItem?: () => void}) => {
    ...
    const submitChanges = async () => {
        try {
            await editTheaterItem(item.id, {
                title, posterUrl, category, featured, engine, sourceUrl, episode, description, featuredPictureUrl
            });
            onSubmit && onSubmit();
    ...
    const createItem = async () => {
        try {
            await createTheaterItem({title, posterUrl, category, featured, engine, sourceUrl, description, featuredPictureUrl});
            deleteCreateItem!();
            onSubmit && onSubmit();
            return notificationStore.addNotification({
                ...defaultNotificationOpts,
    ...
    return (
        <div className={classNames("my-3  text-black p-4", create ? "bg-coolgray" : "bg-black")}>
            ...
            <input value={episode} onChange={e => setEpisode(Number(e.target.value))} className="p-2 rounded w-full bg-spaceblack text-lightgray"></input>
            <div>
