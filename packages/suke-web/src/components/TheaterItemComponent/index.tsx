    ...
    return (
        <div 
            ...
            className={classNames(
                ...
                showSchedules ? "absolute z-20 w-72 md:-left-10" : 'z-10 w-72 md:w-56'
            )} 
        ...
        >
            ...
            <div onClick={close} className={classNames("bg-greatblack fixed top-0 h-100 opacity-70 hover:bg-darkblack duration-200 z-20", showSchedules ? "w-72 opacity-80" : "w-64 md:w-52")} ></div>
            <div onClick={close} style={{backgroundImage: `url("${item.posterUrl}")`}} className={classNames("bg-no-repeat bg-cover h-full mr-10 text-center", showSchedules ? "w-72 absolute z-10" : "w-64 md:w-52")}>
            </div>
            <div className={classNames("absolute pointer-events-none text-white font-signika text-center transform  z-30", showSchedules ? "top-7 w-72"  : "top-1/2 -translate-y-1/2 w-64 md:w-52")}>
            ...
            {
                item.episode && <div className={classNames("bg-blue absolute font-signika top-0 text-xs tracking-wide px-2 py-1 font-semibold z-30", showSchedules ? "right-0" : "md:right-4 right-8")}>
            ...
            <Button className={classNames("absolute z-40 bottom-2 transform left-1/2 -translate-x-1/2 px-3 font-signika", showSchedules && ButtonText !== '' ? 'block' : 'hidden')} size={1} fontSize={'sm'} fontWeight="normal" backgroundColor={activeSchedule?.state === ScheduleState.Starting ? 'green' : 'coolorange'}>
                {
                    ButtonText
                }
            </Button>
